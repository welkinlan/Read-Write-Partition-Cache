...
enum cache_policy {
    ...
    RWP /* replace with RWP */
};

/* block status values */
#define CACHE_BLK_VALID		0x00000001	/* block in valid, in use */
#define CACHE_BLK_DIRTY		0x00000002	/* dirty block RWP: write-only */

/* cache block (or line) definition */
struct cache_blk_t
{
    //unsigned int partition; /* RWP: 1: read-dirty and read only; 2: write-only */
...
struct cache_set_t
{
    unsigned int predicted_dirty_partition_size; /* RWP: predicted best partition size for dirty lines */
    unsigned int current_dirty_lines_num; /* RWP: current number of ways assigned to dirty lines */
...
cache_create(char *name,		/* name of the cache */
             ...
             unsigned int predicted_dirty_partition_size); /* RWP: predicted dirty partition size for LLC */
