...
cache_create(char *name,		/* name of the cache */
             ...
             unsigned int predicted_dirty_partition_size)
/* RWP: predicted dirty partition size for LLC */
{
    struct cache_t *cp;
    struct cache_blk_t *blk;
    int i, j, bindex;
    ...
    {
        if (!strcmp(name,"ul2")){ /* RWP: initialize cache dirty partition size for LLC */
            cp->sets[i].predicted_dirty_partition_size = predicted_dirty_partition_size;
            cp->sets[i].current_dirty_lines_num = 0;
        }
             ...
             md_addr_t *repl_addr)	/* for address of replaced block */
{
    ...
    char *name = cp->name; /* RWP: cache name */
        ...
        case RWP: /* RWP miss */
            if (cmd == Read){
                if (cp->sets[set].current_dirty_lines_num < /* fewer dirty lines */
                    ...
                    /* picks the LRU line from the dirty partition */
                    repl = cp->sets[set].way_tail;
                    while(repl){
...
cache_hit: /* slow hit handler */
    ...
    /*
    switch (blk->partition){
        case 0:
            if (cmd == Write){
                blk->partition = 2;
                //sets[set].current_dirty_lines_num++;
            }
            else{
                blk->partition = 1;
            }
